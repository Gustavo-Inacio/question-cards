...
let questions = {}; // Agora carregado via fetch
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Carregamento Assíncrono das Perguntas
    try {
        const response = await fetch('perguntas.json');
        if (!response.ok) throw new Error('Falha ao carregar o arquivo de perguntas.');
        questions = await response.json();
    } catch (error) {
        alert("Puxa, tivemos um problema ao carregar as perguntas do Cupido! 🏹 Por favor, verifique sua conexão e tente recarregar a página.");
        console.error("Erro ao carregar perguntas:", error);
        return; // Interrompe a inicialização se as perguntas não carregarem
    }
